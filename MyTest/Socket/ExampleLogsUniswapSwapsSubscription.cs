        ...
        public async Task SubscribeAndRunAsync()
        {
            ...
            try
            {
                ...
                eventSubscription.GetSubscriptionDataResponsesAsObservable().Subscribe(log =>
                {
                    ...
                    var amount0Out = Nethereum.Util.UnitConversion.Convert.FromWei(swap.Event.Amount0Out);
                    var amount1In = Nethereum.Util.UnitConversion.Convert.FromWei(swap.Event.Amount1In);


                    var amount0In = Nethereum.Util.UnitConversion.Convert.FromWei(swap.Event.Amount0In);
                    var amount1Out = Nethereum.Util.UnitConversion.Convert.FromWei(swap.Event.Amount1Out);
